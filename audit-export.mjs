...
// seed
await p.evaluate(()=>{const n=Date.now();localStorage.setItem('margin.v1',JSON.stringify({
...
ok('backup filename '+j.name, /^margin-backup-\d{4}-\d{2}-\d{2}\.json$/.test(j.name));
...
ok('markdown filename '+m.name, /^margin-\d{4}-\d{2}-\d{2}\.md$/.test(m.name));
...
ok('anki filename '+a.name, /^margin-anki-\d{4}-\d{2}-\d{2}\.txt$/.test(a.name));
...
await p.evaluate(()=>localStorage.setItem('margin.v1',JSON.stringify({books:[],words:[],quotes:[],notes:[]})));
await p.reload();
...
const back=await p.evaluate(()=>JSON.parse(localStorage.getItem('margin.v1')));
ok('restore brings words/quotes/notes', back.words.length===1&&back.quotes.length===1&&back.notes.length===1);
// idempotent
await p.click('text=Backup & export');
await p.setInputFiles('#imp','/tmp/bk.json'); await p.waitForTimeout(600);
const again=await p.evaluate(()=>JSON.parse(localStorage.getItem('margin.v1')).words.length);
...
ok('junk file rejected, data intact', await p.evaluate(()=>JSON.parse(localStorage.getItem('margin.v1')).books.length===1));
